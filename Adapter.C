...
bool preciceAdapter::Adapter::configFileRead()
{
    ...
    try
    {
	    ...
	    if (module == "AITS") AITSenabled_ = true;
        }
        ...
        if (AITSenabled_)
        {
            ...
            if (!AITS_->configure(preciceDict))
            {
                return false;
            }
        }
        ...
        if (!CHTenabled_ && !FSIenabled_ && !FFenabled_ && !AITSenabled_) // NOTE: Add your new switch here
        {
...
void preciceAdapter::Adapter::configure()
{
    ...
    try
    {
		...
		if (AITSenabled_) AITS_->addWriters(dataName, interface);
		...
		if (AITSenabled_) AITS_->addReaders(dataName, interface);
